...
<p align="center">
  <!-- 차량 실물 사진 추후 첨부 예정 -->
  <i>차량 실물 사진 · 2-노드 배선 사진 · 미로 주행 사진 추후 첨부 예정</i>
</p>
...
**remote 노드(조종 노드)** 는 **MPU-9255 9축 IMU**(자이로 3축 + 가속도계 3축 + 지자기 3축)를 달고 있으며, 기울기로 차를 조종하고 차량의 회전 각도를 실제 각도로 판정하기 위한 노드입니다. 현재는 **진단 단계**로, I2C 버스 스캔·`WHO_AM_I` 칩 판별·CAN 루프백 자기진단·CAN_ESR 오류 감시·버스오프 소프트웨어 복구까지 구현되어 있고, **자세값 산출과 CAN 송신은 아직 구현 전**입니다.
...
| **RTOS** | FreeRTOS (CMSIS-RTOS v1) — vehicle 태스크 4개(우선순위 분리, 뮤텍스·Mail Queue), remote 태스크 2개 |
...
| **9축 IMU (remote)** | MPU-9255 (자이로·가속도계·지자기 각 3축), I2C1 Fast Mode 400kHz, `WHO_AM_I`=0x73으로 칩 판별 완료 — **자세값 산출·송신은 미구현** |
...
| 진행 중 | CAN 물리 배선 문제 추적([13-3](#13-3-can-통신-두절-원인-분리--루프백-자기진단)), IMU 자세값 산출·`0x120` 송신 |
...
- **CAN 루프백 자기진단** : 부팅 시 트랜시버와 버스 없이 자기가 보낸 프레임을 자기가 되받아, MCU 안쪽 설정(클럭·비트타이밍·필터·인터럽트)이 정상인지 먼저 증명합니다. 상세는 [13-3](#13-3-can-통신-두절-원인-분리--루프백-자기진단) 참고
- **CAN 오류 감시 및 버스오프 복구** : CAN_ESR의 송신·수신 오류 카운터(TEC/REC), 마지막 오류 코드(LEC), 버스오프·오류수동·오류경고 플래그를 주기적으로 갱신하고, 버스오프에 빠지면 소프트웨어로 복구합니다.
- **아직 구현되지 않은 것** : MPU-9255의 자세값(roll/pitch/yaw) 읽기, 상보필터, 지자기 캘리브레이션, 그리고 자세값을 CAN `0x120`으로 보내는 송신부는 **모두 미구현**입니다.
...
|---|---|---|---|
| **remote** (조종 노드) | `remote/` · `Project03_Remote` | 9축 IMU 자세 측정, 모드·조향 명령 송신 (**송신부 미구현, 진단 단계**) | 0x100 ~ 0x1FF |
| **vehicle** (차량 노드) | `vehicle/` · `Project03_Vehicle` | 자율주행·수동주행·모터 구동, 상태·거리값 브로드캐스트 | 0x200 ~ 0x2FF, 0x3F0 |

> 기존에는 이 자리에 **NUCLEO 마스터 노드(Node1)** 를 두는 구성이었고, vehicle 노드의 `0x100`/`0x110` 수신 처리 코드도 그 전제로 작성되어 지금까지 남아 있습니다. 지금은 그 자리를 **remote 노드**가 대신하며, remote가 실제로 이 두 ID를 송신하는 코드는 아직 작성하지 않았습니다.
...
|---|---|---|---|---|---|
| **0x100** | MODE_CMD | remote → vehicle | **미구현** | 구현 | `data[0]`이 1이면 AUTO, 그 외는 MANUAL. **전환 시 항상 모터 정지를 먼저 수행** |
| **0x110** | MANUAL_CMD | remote → vehicle | **미구현** | 구현 | `data[0]`에 'F'/'B'/'L'/'R'/'S' 방향 문자 |
| **0x120** | IMU_ATTITUDE | remote → vehicle | **미구현** | **미구현** | 9축 IMU 자세값(roll/pitch/yaw). **설계만 확정, 양쪽 다 코드 없음** |
| **0x200** | VEHICLE_STATUS | vehicle → 전체 | 구현 | — | 100ms 주기, 주행모드 + 좌·전·우 거리값 |
| **0x3F0** | HEARTBEAT | vehicle → 전체 | **미구현** | — | **ID만 정의되어 있고 송신 코드 없음** |

즉 현재 실제로 버스에 흐르도록 코드가 갖춰진 것은 **`0x200` 하나뿐**입니다. vehicle 노드는 `0x100`/`0x110`을 받으면 곧바로 처리하도록 준비되어 있으나, 그 명령을 보내는 remote 쪽 코드가 아직 없습니다.

> **코드상의 이름은 아직 예전 것입니다** — `mcal_can.h`의 매크로는 NUCLEO 마스터 구성 시절의 이름(`MCAL_CAN_ID_MASTER_MODE_CMD`, `MCAL_CAN_ID_MASTER_MANUAL_CMD`, `MCAL_CAN_ID_SLAVE_STATUS`, `MCAL_CAN_ID_SLAVE_HEARTBEAT`)을 그대로 쓰고 있습니다. ID 값은 위 표와 동일하며, 이름만 `REMOTE_`/`VEHICLE_` 기준으로 정리하는 작업이 남아 있습니다.
...
**0x120 IMU_ATTITUDE** (DLC 8) — **설계만 확정된 계획이며 아직 구현되지 않았습니다.**

| 바이트 | 내용 | 형식 |
|---|---|---|
| data[0~1] | roll (좌우 기울기) | int16, **0.1도 단위** (예: -23.4도 → -234) |
| data[2~3] | pitch (앞뒤 기울기) | int16, 0.1도 단위 |
| data[4~5] | yaw (방위) | int16, 0.1도 단위 |
| data[6] | 상태 플래그 | 캘리브레이션 완료 여부 등 |
...
실선은 현재 구현되어 동작하는 경로, 점선은 **아직 구현되지 않은 계획 경로**입니다.

```mermaid
flowchart LR
    subgraph REMOTE["remote 노드 (조종·센서)"]
        IMU["MPU-9255 9축 IMU<br/>I2C1 400kHz, 주소 0x68"]
        MCU_R{{"STM32F103C8T6<br/>FreeRTOS 태스크 2개"}}
    ...
    IMU -. "자세값 읽기 (미구현)" .-> MCU_R
    MCU_R --> DBG
    MCU_R -. "0x100 모드 · 0x110 수동 · 0x120 자세 (송신 미구현)" .-> BUS

    US --> MCU_V
    BT -- "F/B/L/R/S · A/P" --> MCU_V
    BUS -. "0x100 · 0x110 (vehicle 수신부는 구현, 실제 수신 없음)" .-> MCU_V
    MCU_V -- "0x200 주행모드 + 좌/전/우 거리값 (100ms)" --> BUS
...
위 4계층 구조(ASW/RTE/BSW/Core) 설명은 `vehicle/` 노드에 해당합니다. `remote/` 노드는 아직 **진단 코드가 CubeMX 기본 구조(`Core/`) 안에 그대로 들어 있는 상태**입니다. IMU 자세값 산출과 CAN 송신을 구현하는 단계에서 같은 계층 규칙(`ECU_MPU9255_...`, `MCAL_CAN_...` 등)으로 분리할 예정입니다.
...
### 11-2. remote 노드 태스크 구성 (2개)

remote 노드는 아직 진단 단계라 태스크가 2개뿐입니다. FreeRTOS 힙은 6144바이트입니다.
...
- **원인을 가르는 시험을 먼저 만든다 (CAN 루프백 자기진단)** : 통신이 안 될 때 배선·트랜시버·설정·상대 노드를 차례로 바꿔 보는 대신, **트랜시버도 버스도 없이 자기 프레임을 자기가 받아보는 시험**을 먼저 만들었습니다. 이 시험이 성공하면 MCU 안쪽(클럭·비트타이밍·필터·인터럽트)은 정상임이 증명되어, 남은 원인이 바깥으로 확정됩니다. 후보를 절반으로 줄이고 시작하는 셈입니다.
...
### 13-2. 알 수 없는 문자가 수동주행을 정지시킴

- **증상** : 블루투스로 방향키를 눌러 주행 중이던 차가 갑자기 멈추고, 그 뒤로 계속 멈춰 있었습니다.
- **원인** : `ASW_Manual_ApplySerialChar()`가 `'A'`/`'P'`가 아닌 **모든 문자를 그대로 `s_lastCmd`에 저장**하고 있었습니다. 그런데 `ASW_Manual_ProcessControl()`의 switch는 `F/B/L/R/S`만 처리하고 나머지는 `default:` → `RTE_Motor_Stop()`으로 떨어집니다. 시중 RC카 블루투스 앱은 방향 문자 외에도 **속도 단계(`'0'`~`'9'`), 전조등(`'W'`), 경적(`'V'`)** 같은 부가 문자를 함께 보내므로, 그중 하나만 들어와도 주행이 멈추고 직전 방향 명령까지 덮어써져 다시 방향키를 누르기 전까지 멈춘 채로 있었습니다.
- **수정 전** : `'A'`/`'P'`가 아니면 무조건 주행 명령으로 받아들였습니다.
...
- **수정 후** : **"링크 생존 확인"과 "주행 명령 변경"을 분리**했습니다. 아는 명령 문자일 때만 `s_lastCmd`를 바꾸고, 모르는 문자는 주행 명령을 건드리지 않되 **타임스탬프만 갱신**합니다. 바이트가 도착했다는 사실 자체가 블루투스 링크가 살아 있다는 증거이기 때문입니다.

```c
/* 주행 명령으로 "인정"하는 문자인지 판별한다.
   여기 목록에 없는 문자는 전부 모르는 문자로 취급해 주행 명령을 바꾸지 않는다. */
static uint8_t ASW_Manual_IsDriveCmdChar(uint8_t ch)
{
    ...
    {
        /* 모르는 문자(속도 단계 '0'~'9', 전조등 'W', 경적 'V' 등).
         * 주행 명령은 절대 건드리지 않고 무시한다. 다만 바이트가 도착했다는 것은
         * 블루투스 링크가 살아있다는 증거이므로 타임스탬프만 갱신해 준다.
         *
         * [중요] 반드시 이미 s_cmdSource == SERIAL인 경우에만 갱신한다.
         *  여기서 SERIAL로 새로 설정해버리면, 아직 방향 명령을 한 번도 받은 적
         *  없는 부팅 직후(NONE) 상태에서 잡음 바이트 하나만으로 타임아웃 감시가
         *  시작되어 버린다. 그건 의도한 동작이 아니다. */
...
- **결과** : 부가 문자가 주행을 끊지 않게 되었고, 오히려 그 문자들이 **무응답 타임아웃을 갱신해 주는 역할**을 하게 되었습니다. [13-1](#13-1-블루투스-연결-끊김-시-안전정지-미동작)에서 남은 문제였던 "버튼을 5초 넘게 누르고 있으면 멈춘다"도, 앱이 부가 문자를 함께 보내는 경우라면 그만큼 완화됩니다. 또한 `ProcessControl()`의 `default:`는 이제 도달하지 않는 방어 코드가 되었는데, CAN 경로는 상대가 보낸 값을 그대로 넣으므로 **상대 노드가 이상한 값을 보냈을 때 안전하게 멈추는 최후의 보루** 역할로 그대로 남겨 두었습니다.
...
- **접근** : 후보를 하나씩 지우는 대신 **내부와 외부를 한 번에 갈라내는 시험**을 먼저 만들었습니다. CAN 컨트롤러의 **루프백 모드**는 트랜시버도 바깥 배선도 없이 자기가 내보낸 프레임을 자기가 되받는 시험 모드입니다. 여기서 성공하면 클럭·비트타이밍·필터·인터럽트 같은 **MCU 안쪽 설정이 전부 정상**임이 증명되고, 남은 원인은 바깥뿐으로 확정됩니다. (현재 remote 노드에만 구현되어 있습니다.)
...
| # | 한계 | 현재 상태 | 개선 방향 |
|---|---|---|---|
| **1** | **vehicle 노드의 printf와 블루투스가 USART1 공유** | vehicle 노드는 **USART1 하나만 설정되어 있어 별도 디버그 UART가 없음**. `printf` 출력이 `huart1`으로 나가는데 이것이 HC-06과 같은 포트라, **디버그 로그가 휴대폰 앱으로 전송됨**. 게다가 9600bps에서 한 줄(약 30바이트)에 30ms 넘게 블로킹 송신하므로 50ms 제어주기를 심하게 잠식. 현재는 디버그 관찰을 STM32CubeIDE **Live Expressions**로 대체 중. remote 노드는 블루투스가 없어 USART1을 온전히 디버그(115200bps)로 쓸 수 있으나, **USB-TTL 어댑터가 없어 실제로는 그 출력도 보지 못하고 있음** | vehicle은 USART2를 디버그 전용으로 분리하거나 SWO(Serial Wire Output) 트레이스로 로그 경로를 완전히 분리. 로그 자체도 블로킹 송신 대신 DMA·인터럽트 기반 링버퍼로 전환. remote는 USB-TTL 어댑터 확보 |
...
| **3** | **FAULT 상태코드·하트비트 미사용** | `MCAL_CAN_STATE_FAULT`(2)가 enum에 정의만 되어 있고 **이 값을 설정하는 코드 경로가 없음**. 실제 브로드캐스트되는 값은 MANUAL(0)/AUTO(1)뿐이며, `VEHICLE_STATUS`의 Fault 플래그 바이트(data[5])도 0 고정. 생존 신호 `HEARTBEAT`(0x3F0)도 **ID만 정의되고 송신 코드가 없음**. 버스오프·오류 카운터를 감지하는 진단 변수(`g_vdiag_*`)는 갖춰졌으나 **아직 FAULT 상태로 연결되어 있지 않음** | 이미 수집 중인 CAN 진단 값을 FAULT 판정에 연결하고, 센서 무응답·모터 무반응 등 자체 진단 항목을 추가해 상대 노드에 보고. 하트비트(0x3F0)를 주기적으로 송신해 노드 생존을 감시. 정의만 되고 쓰이지 않는 `MCAL_CAN_TIMEOUT_MASTER_MS`도 이 진단 로직에 연결 |
| **4** | **2-노드 CAN 통신 미확립** | remote 노드는 루프백 자기진단으로 MCU 쪽 설정이 정상임을 확인했으나, **실제 버스로는 아직 한 프레임도 오가지 않음**(vehicle 노드에는 루프백 자기진단이 아직 없음). 트랜시버 모듈의 GND 부상(약 0.8V)이 확인되어 배선 점검 중([13-3](#13-3-can-통신-두절-원인-분리--루프백-자기진단)) | GND 결선을 바로잡고 종단저항 120Ω 2개(양 끝)를 확인. 통신이 붙으면 remote의 `0x100`/`0x110` 송신부를 구현해 원격 조종 경로를 완성 |
| **5** | **IMU 자세값 미구현** | remote 노드는 MPU-9255를 **인식하는 단계까지만** 완료(`WHO_AM_I`=0x73 확인). 자세값(roll/pitch/yaw) 읽기, 자이로와 가속도계를 합치는 상보필터, 지자기 캘리브레이션, `0x120` 송신이 **모두 미구현**. 따라서 vehicle의 회전 각도 판정은 여전히 시간 기반 추정(`ASW_AUTO_PIVOT_EXPECT_MS` 500ms)에 의존 | 가속도계로 기울기의 절대 기준을 잡고 자이로로 짧은 시간의 변화를 따라가는 상보필터를 구현. yaw는 지자기 센서(AK8963, 주소 0x0C)로 드리프트를 보정. 자세값이 들어오면 피벗 종료 판정을 시간이 아닌 **실제 회전각 기준**으로 대체 |

> 위 항목들은 성격이 둘로 나뉩니다. 1~3번은 **"동작은 하지만 설계 의도까지는 도달하지 못한"** 부분입니다. 특히 1번(디버그 UART 부재)은 개발 초기 핀 배치 단계에서 디버그 경로를 확보해두지 않으면 나중에 되돌리기 어렵다는 점을, 2번(센서 갱신 지연)은 센서 구성이 제어 알고리즘의 상한을 결정한다는 점을 확인한 사례로, 차기 프로젝트의 초기 설계 체크리스트에 반영할 항목입니다.
>
> 4~5번은 **아직 구현·해결되지 않은 진행 중인 작업**입니다. 이 문서에서 "미구현"으로 표기된 기능은 모두 여기에 해당하며, 동작하는 것처럼 서술하지 않았습니다.
...
**디버그 관찰** — 로그를 눈으로 볼 수 없는 환경이므로([14. 한계](#14-한계-및-차기-프로젝트-반영) 1번 참고), 내부 변수는 STM32CubeIDE의 **Live Expressions** 창에 변수 이름을 그대로 적어 넣어 관찰합니다.
