...
| **과회전 방지** | 피벗 진입 시점 기준값이 신뢰할 만했고(`s_pivotYawBaseOk`), **지금도** yaw가 신선하며(400ms 이내 수신), 변화량이 90도 이상이고 전방 30cm 초과 | 느슨한 거리 기준으로 DRIVE 복귀 |
| **시간 폴백** | 위 세 조건 중 하나라도 아니고, 500ms 경과 후 전방 30cm 초과 | 기존 시간 추정으로 DRIVE 복귀 |
...
**"한 번 유효했다"와 "지금도 살아있다"는 다른 질문이라 둘 다 확인합니다.** `ASW_ImuAttitude_IsValid()`는 부팅 후 한 번 캘리브레이션되면 계속 1을 반환하는 래치라, 피벗 진입 시점의 유효성만 저장하고 끝까지 믿으면 remote의 CAN 링크가 피벗 도중 끊겨도(전원 차단, 버스오프 등) 값이 마지막 수신값에 고정된 채 "유효함"으로 남습니다. 그러면 기준값과 현재값이 같아 변화량이 항상 0에 가깝고, 90도 조건은 영원히 만족되지 않는데 시간 폴백은 이미 각도 분기에 갇혀 접근조차 안 됩니다. 그래서 공유 모듈에 마지막 수신 시각을 추가해(`ASW_ImuAttitude_IsFresh(400)`) **피벗 진입 시점의 "기준값이 신뢰할 만했는가"는 한 번만 저장하되, 매 제어 주기 "지금도 신선한가"를 추가로 확인**합니다. 둘 다 참일 때만 각도 판정을 쓰고, 어느 하나라도 아니면 그 주기부터 시간 폴백으로 넘어갑니다 — 링크가 죽으면 즉시(다음 주기부터) 폴백으로 빠지고, 피벗 시작 시점에 아직 캘리브레이션 전이었다면 그 피벗은 끝까지 시간 기준만 씁니다(진입 시점 기준값이 없는 상태에서 도중에 값이 들어와도 갑자기 각도 판정으로 승격되지 않음 — 그 값을 기준 0도와 비교하면 오판할 수 있기 때문).
...
yaw·캘리브레이션 상태·마지막 수신 시각은 공유 모듈 `asw_imu_attitude`에 저장합니다. CAN 수신 태스크에서는 정수 대입만 수행하고, 부동소수점 변환은 50ms 제어 태스크가 값을 읽을 때 처리해 가장 높은 우선순위의 수신 경로를 짧게 유지합니다. 피벗에 진입하면 그 시점의 유효성만 한 번 저장하고, 이후에는 매 주기 신선도(`IsFresh`)를 다시 확인하면서 상대 변화량의 절댓값이 90도에 도달했는지 봅니다 — 자세한 이유는 5-6과 13-6을 참고하세요.

roll·pitch 명령 출처 전환에도 같은 원칙의 게이트가 있습니다: 기울기가 데드존 안(평평함)이면 **이미 IMU가 활성 출처인 경우에만** 갱신을 반영해 즉시 정지시키고, 다른 출처(SERIAL 등)가 활성일 때는 갱신을 건너뛰어 평평하게 놓인 remote가 조종권을 가로채지 못하게 합니다 — 13-7 참고.
...
- **후속 발견** : 이 수정은 "피벗 진입 시점에 유효했다"만 보고 끝까지 그 판정을 믿는다는 새로운 가정을 만들었고, 이 가정 자체가 다음 문제(13-6)의 원인이 됩니다.

<br>

### 13-6. IMU 유효성 래치가 CAN 링크 단절 이후에도 유지되어 페일세이프가 영구 비활성화되는 문제

- **문제** : 13-5의 수정은 피벗 진입 시점의 유효성만 한 번 저장하고 그 피벗이 끝날 때까지 그대로 믿습니다. `ASW_ImuAttitude_IsValid()`는 remote가 한 번이라도 캘리브레이션된 프레임을 보내면 계속 1을 반환하는 부팅 래치일 뿐, "지금 링크가 살아있다"는 뜻이 아닙니다. remote의 CAN 링크가 피벗 도중 끊기면(전원 차단, 버스오프 등) yaw 값이 마지막 수신값에 고정된 채 유효성은 계속 1로 남고, 기준값과 현재값이 같은 값이라 변화량이 항상 0에 가까워 90도 조건이 영원히 만족되지 않습니다. 이 상황을 잡아야 할 시간 폴백은 `else` 분기에 있어 유효성이 1인 한 접근조차 되지 않습니다 — 정확히 이 페일세이프가 막으려던 상황(과회전 U턴)에서 페일세이프 자체가 조용히 죽어 있는 셈입니다.
- **해결** : 공유 모듈 `asw_imu_attitude`에 마지막 수신 시각을 추가하고 `ASW_ImuAttitude_IsFresh(maxAgeMs)`를 새로 만들었습니다(0x120이 100ms 주기이므로 400ms = 약 4프레임 허용). 피벗 진입 시점의 "기준값이 신뢰할 만했는가"(`s_pivotYawBaseOk`)는 여전히 한 번만 저장하지만, 매 제어 주기 `IsFresh()`를 추가로 확인해서 **두 조건을 모두 만족할 때만** 각도 판정을 씁니다. 어느 한쪽이라도 아니면 그 주기부터 시간 폴백으로 넘어갑니다.

  **수정 후 (최종)**
  ```c
  /* PIVOT 진입 시 "기준값이 신뢰할 만했는가"만 한 번 저장 */
  s_pivotYawBaseOk   = ASW_ImuAttitude_IsValid() &&
                       ASW_ImuAttitude_IsFresh(ASW_AUTO_PIVOT_IMU_MAX_AGE_MS);
  s_pivotYawStartDeg = ASW_ImuAttitude_GetYawDeg();

  /* 매 주기 : 기준값도 좋았고, 지금도 살아있어야 각도 판정 사용 */
  if (s_pivotYawBaseOk &&
      ASW_ImuAttitude_IsFresh(ASW_AUTO_PIVOT_IMU_MAX_AGE_MS))
  {
      float turnedDeg = AswAuto_PivotYawDeltaDeg();  /* 부호 있는 상대각 */
      overturned = (fabsf(turnedDeg) >= ASW_AUTO_PIVOT_EXPECT_DEG);
  }
  else
  {
      overturned = (s_turnElapsedMs >= ASW_AUTO_PIVOT_EXPECT_MS);
  }

  if (overturned && (dist_front > ASW_AUTO_DIST_WARNING_CM))
  {
      RTE_Motor_Stop();
      AswAuto_EnterDrive();
      return;
  ...
  ```
- **배운 점** : "한 번 유효했다"와 "지금도 살아있다"는 서로 다른 질문입니다. 부팅 래치처럼 한 번 참이 되면 계속 참인 플래그는 초기화 여부를 확인하는 데는 맞지만 링크 생존 확인에는 쓸 수 없습니다. 페일세이프를 다른 분기의 `else`로 가둬 두면, 그 분기로 넘어갈 조건 자체가 영원히 거짓이 되는 순간 페일세이프 전체가 죽는다는 것도 다시 확인했습니다.

<br>

### 13-7. remote가 켜져 있기만 해도 폰 앱 수동조종을 가로채는 문제

- **문제** : remote는 부팅 직후부터 100ms마다 무조건 CAN `0x120`을 보냅니다. vehicle의 CAN 수신 처리는 이 프레임을 받을 때마다 조건 없이 명령 출처를 IMU로 바꿨습니다. 그래서 블루투스 폰 앱으로 운전 중이어도 remote가 근처에 켜진 채 평평하게 놓여 있으면, 100ms마다 출처가 IMU로 되돌아가고 기울기가 데드존 안이라 매번 정지 명령이 내려져 폰 앱 조종이 사실상 불가능했습니다.
- **해결** : `ComputeTiltDuty()`가 이미 하고 있는 데드존(양쪽 축 5도 미만) 판정 결과가 실제로 0이 아닐 때, 또는 **IMU가 이미 활성 출처일 때만** 명령 출처·타임스탬프·duty를 갱신하도록 게이트를 추가했습니다. "이미 IMU가 활성 출처일 때"도 통과시키는 이유는, 손을 놓아 평평해지는 순간까지 완전히 무시해버리면 즉시 멈춰야 할 상황이 1000ms 무응답 타임아웃까지 늦춰지기 때문입니다 — 그래서 IMU 조종 중 손을 놓으면 여전히 한두 주기(약 150ms) 안에 멈춥니다.
- **배운 점** : 여러 명령 출처가 "마지막으로 받은 쪽이 이긴다" 규칙을 공유할 때는, 주기적으로 계속 보내는 쪽이 사실상 항상 이깁니다. 데드존처럼 "의미 있는 입력인지"를 출처 전환의 조건에 넣어야 하고, 이미 활성인 출처의 종료 신호(여기서는 평평해짐 = 정지)는 예외로 통과시켜야 응답성이 죽지 않습니다.

<br>

### 13-8. remote 부팅 시 I2C 버스 경합

- **문제** : 부팅 시 I2C 스캔(`StartDefaultTask`)과 IMU 초기화(`StartImuTask`)가 같은 우선순위로 동시에 생성되어, 둘 다 같은 `hi2c1` 핸들을 동시에 건드릴 수 있었습니다. STM32 HAL의 `__HAL_LOCK`은 "확인 후 설정" 방식이라 선점에 안전하지 않아서, 재시도로 넘어가는 "깨끗한 HAL_BUSY"만 일어난다는 보장이 없었습니다 — 실제로 두 태스크가 동시에 같은 레지스터를 건드릴 위험이 있었습니다.
- **해결** : 바이너리 세마포어를 추가해 `StartDefaultTask`가 I2C 스캔을 끝낸 뒤에만 신호를 보내고, `StartImuTask`는 `Mpu9255_Init()`을 부르기 전에 그 신호를 기다리도록 순서를 강제했습니다. (CMSIS-RTOS v1의 바이너리 세마포어는 생성 직후 이미 토큰을 하나 쥔 상태로 만들어지므로, 만들자마자 한 번 비워 둬야 실제로 대기가 걸립니다 — 안 그러면 아무 효과 없는 수정이 됩니다.)
- **배운 점** : 우선순위가 같은 두 태스크가 부팅 시점에 같은 하드웨어 자원을 동시에 건드리면 재시도만으로는 경합을 없애지 못하고 감출 뿐입니다. 순서 보장이 필요한 자원은 세마포어 같은 명시적 동기화로 아예 겹치지 않게 만드는 편이 근본적입니다.

<br>

### 13-9. CAN 루프백 자가진단이 일시적 읽기 실패와 진짜 타임아웃을 구분하지 못하는 문제

- **문제** : 루프백 자가진단은 기대한 테스트 프레임이 나올 때까지 수신함을 계속 비웁니다. 그런데 그 도중 `HAL_CAN_GetRxMessage()` 호출이 한 번이라도 실패하면 곧바로 포기했고, 그 결과를 "100ms 동안 아무것도 안 옴"과 같은 결과 코드로 기록해 둘을 구분할 수 없었습니다.
- **해결** : 드레인 도중의 읽기 실패를 별도 결과 코드(5)로 분리하고, 실패해도 남은 시간 안에서는 계속 대기하도록 바꿨습니다 — 실패한 프레임 바로 뒤에 실제 테스트 프레임이 와 있을 수 있기 때문입니다.
- **배운 점** : 자가진단 코드가 "성공/실패"만 남기면 서로 다른 원인의 실패가 같은 숫자 뒤에 숨어버립니다. 원인별로 결과 코드를 나눠야 실제로 디버깅에 쓸 수 있습니다.
