...
/* 자이로 영점 측정이 끝났으면 1 (CAN 상태 바이트의 0번 비트로 나간다).
   [이 비트가 약속하는 것]
   단순히 "I2C 읽기가 성공했다"가 아니라 "센서가 실제로 깨어서 값을 내놓는 것을
   확인한 뒤 영점을 구했다"는 뜻이다. 잠든 MPU 도 HAL_OK 와 함께 0 을 돌려주므로
   읽기 성공만으로 이 비트를 세우면 자고 있는 센서의 0 을 믿어도 되는 각도로
   내보내게 된다. 그래서 Mpu9255_CalibrateGyro() 가 표본이 전부 0 인지까지 보고
   나서야 세운다. */
...
/**
  ...
  *
  *         [읽기가 성공했다는 것만으로는 센서가 깨어 있다고 말할 수 없다]
  *         잠든 MPU 는 I2C 응답 자체는 멀쩡히 해 주면서 모든 레지스터를 0 으로
  *         돌려준다. HAL 은 이것을 오류가 아니라 HAL_OK 로 알려 준다.
  *         그래서 초기화 2단계의 깨우기 쓰기가 소리 없이 실패했더라도 이 함수는
  *         200개를 전부 "잘 읽은 표본"으로 세고, 영점을 정확히 0.0 으로 잡고,
  *         "쟀다"는 표시까지 세운 채 성공을 돌려주게 된다. 그 뒤로는 자고 있는
  *         센서에서 나온 0 을 가지고 계산한 각도가, 받는 쪽에는 믿어도 되는
  *         값으로 표시되어 나간다. 읽기 성공 여부만으로는 이것을 가려낼 수 없다.
  *         살아 있는 자이로는 가만히 놓아 두어도 늘 몇 LSB 씩 잡음이 흔들린다.
  *         200번을 내리 읽었는데 세 축이 모두 정확히 0 이었다면 그것은 아주
  *         조용한 것이 아니라, 자고 있거나 애초에 거기 붙어 있지 않은 것이다.
  *         (배선 불량, SPI 모드에 갇힌 칩, 끝내 깨어나지 못한 모듈이 다 이 모습이다)
  *         그래서 이 경우도 영점을 구하지 못한 것으로 본다.
  *
  * @retval 1 = 영점을 구했고, 센서가 실제로 살아 움직이는 값을 내놓고 있음
  * @retval 0 = 한 표본도 못 읽었거나(I2C 불통), 읽기는 다 됐지만 모든 표본이
  *             세 축 모두 정확히 0 이었음(센서가 잠들어 있음). 어느 쪽이든
  *             영점은 0 으로 두고 "쟀다"는 표시를 세우지 않는다.
  */
static uint8_t Mpu9255_CalibrateGyro(void)
{
  uint8_t  buf[6];
  ...
  uint8_t  any_nonzero = 0u;   /* 어느 한 표본에서든 세 축 중 하나라도 0 이 아니었으면 1 */
  int16_t  raw_x;              /* 이번 표본의 원시값. 합계에 넣기 전에 0 인지 따로 보려고 담아 둔다 */
  int16_t  raw_y;
  int16_t  raw_z;
  ...
  for (i = 0u; i < MPU9255_CAL_SAMPLE_CNT; i++)
  {
    ...
    if (Mpu9255_ReadRegs(MPU9255_REG_GYRO_XOUT_H, buf, 6u, 1u) != 0u)
    {
      ...
      raw_x = Mpu9255_Be16(&buf[0]);
      raw_y = Mpu9255_Be16(&buf[2]);
      raw_z = Mpu9255_Be16(&buf[4]);
      ...
      /* [합계가 아니라 표본 하나하나를, 그것도 축마다 따로 본다]
         "합이 0 인가" 로 판단하면 안 된다. 살아 있는 자이로의 잡음은 +와 -를
         오가므로 200개를 더한 값이 우연히 정확히 0 이 되는 일이 실제로 일어나고,
         그러면 멀쩡한 센서를 잠들었다고 몰아세워 초기화를 실패시키게 된다.
         반대로 잠든 센서는 어느 표본의 어느 축을 집어도 예외 없이 0 이다.
         그래서 표본마다 세 축을 따로 보고 "0 이 아닌 값을 한 번이라도 본 적이
         있는가" 하나만 남긴다. 한 번 서면 다시 지울 일이 없는 표시라,
         200번을 도는 동안 늘어나는 일은 비교 세 번뿐이다. */
  ...
  /* [원인이 다른 두 실패를 한 갈래로 묶어 둔 이유]
     뒤처리가 완전히 같기 때문이다. 영점을 0 으로 되돌리고, "쟀다"는 표시를
     세우지 않고 물러난다. 같은 네 줄을 두 군데에 적어 두면 나중에 한쪽만
     고치는 일이 반드시 생기므로, 조건만 둘로 갈라 한 자리에 모았다.
       good_cnt == 0    : I2C 가 200번 내내 한 번도 대답하지 않았다.
                          배선이 끊겼거나 버스가 죽은 것이다.
       any_nonzero == 0 : 대답은 200번 다 했는데 세 축이 모두 정확히 0 이었다.
                          살아 있는 자이로에서는 나올 수 없는 모습이고,
                          깨우기 쓰기가 먹히지 않아 칩이 아직 자고 있다는 뜻이다.
                          앞의 경우와 달리 I2C 자체는 멀쩡하므로, 이쪽은
                          HAL 이 아무 오류도 내지 않은 채 조용히 지나간다.
     어느 쪽이든 CAN 상태 바이트의 0번 비트가 계속 0 으로 나가므로,
     받는 쪽에서도 이 각도를 믿으면 안 된다는 것을 알 수 있다. */
  if ((good_cnt == 0u) || (any_nonzero == 0u))
  {
    s_gyroXBias     = 0.0f;
    s_gyroYBias     = 0.0f;
    s_gyroZBias     = 0.0f;
...
/**
  ...
  *
  *         [모든 쓰기가 HAL_OK 였는데도 0 이 돌아올 수 있다]
  *         2단계의 깨우기 쓰기가 소리 없이 먹히지 않으면 칩은 계속 자고 있는데,
  *         잠든 MPU 는 I2C 응답은 정상으로 하면서 모든 레지스터를 0 으로 돌려준다.
  *         그래서 이 함수의 1~3단계만으로는 센서가 깨어났는지 알 길이 없고,
  *         4단계의 영점 측정이 실제로 흔들리는 값을 보았는지까지 확인해 준다.
  *         즉 여기서 0 이 돌아왔다면 I2C 거래가 실패한 경우뿐 아니라
  *         "거래는 다 됐는데 센서가 자고 있더라"인 경우도 포함된다.
  *
...
/**
  ...
  *
  *         [1 이 보증하는 범위]
  *         영점을 구했다는 것에 더해, 그 200개 표본 가운데 0 이 아닌 값이
  *         적어도 하나는 있었다는 뜻이다. 잠든 MPU 는 I2C 응답을 정상으로 하면서
  *         0 만 돌려주므로, 읽기 성공만으로 1 을 세우면 자고 있는 센서를
  *         "각도를 믿어도 되는 상태"로 잘못 알리게 된다. 그래서 이 비트는
  *         센서가 실제로 움직이는 값을 내놓는 것까지 확인한 뒤에만 선다.
  * @retval 1 = 끝났음(센서가 깨어 있고 각도를 믿어도 된다)
  * @retval 0 = 아직이거나, 측정에 실패했거나, 센서가 잠들어 0 만 내놓고 있음
  */
