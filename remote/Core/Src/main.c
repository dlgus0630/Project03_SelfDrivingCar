...
/* ===== 루프백 자기진단 결과 =====
   ...
   남은 원인은 바깥(트랜시버·배선·종단저항·상대 노드)뿐으로 좁혀진다.
     ...
     4 = 시험용 프레임이 안 되돌아옴 (남의 프레임은 들어왔지만,
         내가 보낸 0x7FF 시험용 프레임은 100ms 안에 되돌아오지 않았다)
     5 = 수신함에서 꺼내기 실패 (수신함에 뭔가 들어오기는 했는데 꺼내는 데
         실패했고, 시험용 프레임도 끝내 되돌아오지 않았다. 3 과 달리
         "아무것도 안 왔다"가 아니므로 배선이 아니라 수신함을 다루는 쪽을 본다) */
...
/* 버스오프에서 소프트웨어로 되살리는 데 온전히 성공한 횟수.
   시도만 하고 실패한 것은 세지 않는다. 실패는 g_diag_can_stuck 이 알려 준다. */
volatile uint32_t g_diag_can_recover_cnt  = 0u;
/* CAN 손잡이(hcan)가 리셋 없이는 되살릴 수 없는 상태에 빠졌으면 1.
   HAL_CAN_Stop() 은 손잡이 상태가 LISTENING 일 때만, HAL_CAN_Start() 는
   READY 일 때만 실제로 하드웨어를 건드린다. 그 밖의 상태에서는 아무 일도
   하지 않고 곧바로 오류만 돌려준다. 그래서 한 번 어긋난 상태로 굳어 버리면
   1초마다 다시 시도해 봐야 매번 헛물만 켤 뿐이고, g_diag_can_recover_cnt 는
   조금도 늘지 않고 그 자리에 멈춰 있는다. 온전히 되살아난 횟수만 세는 값이라
   실패한 시도는 아무리 되풀이해도 반영되지 않기 때문이다.
   Live Expressions 로 지켜볼 때 g_diag_can_stuck 이 1 인데 이 값이 멈춰 있다고
   해서 이상할 것이 없다. "몇 번 시도했는가"가 아니라 "몇 번 되살아났는가"다.
     1 = 그렇게 굳어 버린 상태라 Stop/Start 를 아예 시도조차 못 했거나,
         시도는 했지만 실패했다는 뜻이다. 이때는 MCU 를 리셋해야 풀린다.
     0 = 그런 상태가 아니다.
   지금 형편을 그대로 비추는 값이라, 나중에 복구가 온전히 성공하면 다시 0 이 된다. */
volatile uint8_t  g_diag_can_stuck        = 0u;
/* USER CODE END PV */
